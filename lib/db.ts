...
const dbp=openDB<Store>('cobuy_paths',1,{upgrade(db){
  db.createObjectStore('tasks',{keyPath:'id'});
  db.createObjectStore('shortlist',{keyPath:'id'});
  db.createObjectStore('polls',{keyPath:'id'});
  db.createObjectStore('files',{keyPath:'id'});
  db.createObjectStore('settings',{keyPath:'key'});
  db.createObjectStore('networkLogs',{keyPath:'ts'});
}});
export const db={
  async add(s:any,v:any){return (await dbp).add(s,v)},
  async put(s:any,v:any){return (await dbp).put(s,v)},
  async delete(s:any,k:any){return (await dbp).delete(s,k)},
  async getAll(s:any){return (await dbp).getAll(s) as any}
};
